...
trojango="trojan-go://${uuid}@${domain}:${trgo}/?serverName=${domain}&type=ws&host=${domain}&path=/gandring&encryption=none#$user"
trojanhdua="trojan://${uuid}@${domain}:$thdua?serverName=${domain}&type=http&security=tls&path=gandring#${user}"
...
trojantls="trojan://${uuid}@${domain}:$ttls?type=ws&security=tls&host=$domain&path=%2fgandring&serverName=$domain#${user}"
